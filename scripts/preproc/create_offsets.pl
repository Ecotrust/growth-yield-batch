...
#$WORK_DIR = $ARGV[0];
$data_dir = "..\\FVS\\\BaseRX\\Steep_Slope_Changed";
$data_dir = "..\\FVS\\run";
...
#	while ($i<6)
	{
	    ...
	    system("copy $data_dir\\$line offsets\\$rx\_$i.key");
		...
		if (open(FILE_HAND_OUT, ">offsets\\$rx\_$i.key"))
		{
		    #establish a variable to track offset calc (change the multiplier if period length changes
		    $o_calc = ($i - 1) * 1;
#changed this to 1998 - 2004 for sooes
		...
		{
		    print "Can't open file offsets.\$rx\_$i.key\n";
		    exit(1);
		}
		close(FILE_HAND);
		...
		#still within the offset loop we want to create a
                ...
                #open the file
		if (open(BAT_HAND_OUT, ">offsets\\$rx\_$i.bat"))
		    {
#begin bat file==================================
...
#try to make adjustment for go files (no offsets required)
		if ($rx =~ /GO/ and $i > 1)
		{
		    system("del offsets\\$rx\_$i.bat");
		    system("del offsets\\$rx\_$i.key");
		}
		else
		{
