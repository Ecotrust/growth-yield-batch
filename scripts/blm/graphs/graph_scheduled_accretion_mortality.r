...
noclim45 <- noclim
noclim85 <- noclim
noclim45$rcp <- "RCP 4.5"
noclim85$rcp <- "RCP 8.5"
...
acc <- ggplot(data=clim, aes(x=year, y=acc)) +
      ggtitle("Annual Accretion and Mortality Rates") +
      ...
      ylab(expression(paste("Accretion Rate ", " (", ft^{3} * acre^{-1}, ")"))) +
...
mort <- ggplot(data=clim, aes(x=year, y=mort)) +
      ...
      ylab(expression(paste("Mortality Rate ", " (", ft^{3} * acre^{-1}, ")"))) +
